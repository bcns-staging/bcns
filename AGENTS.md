...
- **OpenSky Network's API (`graphql-api/src/opensky.ts`) is unreachable from Cloud Run** — both `opensky-network.org` and its separate OAuth2 token server `auth.opensky-network.org` time out on connect from Cloud Run's shared egress IPs, confirmed even with valid OAuth2 credentials. This is a network-level block against Google's cloud IP ranges, not a rate-limit/auth problem — no fix short of a non-cloud-flagged egress path. The `/map` "Flights" panel gracefully shows "No live flights available" in production as a result; the code works correctly from a non-blocked network (e.g. local dev).
